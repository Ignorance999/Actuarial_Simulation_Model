...
class Variable(object):
    ...
    def __call__(self, *args, **kwargs):
        if self._fFunction != None:
            ...
            # this error is quite impossible, as function is initilized in __init__
            raise Exception("call a None in variable class!")
    ...
    def fSetfFunction(self, mModule_fFunc, bReInitialize=False):
        ...
        self.lsFuncArgs = [sKey for sKey in inspect.signature(
            self._fFunction).parameters.keys()]
        if bReInitialize:
            self.fReInitialize()
