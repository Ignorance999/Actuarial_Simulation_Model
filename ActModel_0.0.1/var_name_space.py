...
def fRunModel():
    ...
    for sVar in lsVars:
        odTemp = OrderedDict(
            {sKey: odVarRanges[sKey] for sKey in globals()[sVar].lsFuncArgs})
    ...
    for sKey, lValue in GLOBAL_SETTINGS.odOutputFormatsRaw.items():
        ...
        for tTemp in lValue:
            ...
            if sType == "ACCUMULATION":
                bTemp = fGetFirstElementOrderedDict(
                    INPUT.dAllInputs["AccumTable"]).fCheckProdInAccum(sCURR_PRODUCT(), sName)
            elif sType == "PRODUCT":
...
def sCURR_PRODUCT():
    if dCURR_POLICY != "EOF":
        ...
        if sProd in INPUT.dKeyGenTables["ProdTable"].flAllKeys():
            ...
            raise Exception("in Buildins,dCURR_PRODUCT:" +
                            sProd + " is not a valid product")
    else:
        return "EOF"
